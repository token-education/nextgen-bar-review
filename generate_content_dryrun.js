...
// Setup Google AI provider using the key
...
const sampleTopics = [
  {
    ...
    "sub_topic_module_tag": "Dispositive Motions",
    "core_pure_recall_doctrine": "Rule 12 Motion to Dismiss (Timing, Procedures, and Defenses for Failure to State a Claim)",
    "database_sync_token": "REC-CIVI-DISPOSIT"
  }
];
...
async function run() {
  console.log("Starting dry run for atomized micro-cards...");
  let finalResults = [];
  let idCounter = 1;
  ...
  for (let i = 0; i < sampleTopics.length; i++) {
    ...
    try {
      console.log(`Generating micro-cards for: ${topic.core_pure_recall_doctrine}...`);
      ...
      for (const parsed of parsedArray) {
        finalResults.push({
          ...
          rule: topic.core_pure_recall_doctrine, // Keep the parent rule for reference
...
`;
  ...
  console.log(`Dry run complete! Wrote ${finalResults.length} micro-cards to src/data/mockTopics.ts`);
}
